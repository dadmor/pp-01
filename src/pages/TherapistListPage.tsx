// src/pages/TherapistListPage.tsx
...
export default function TherapistListPage() {
  ...
  // Funkcja do prefetchowania danych terapeuty
  const prefetchTherapist = (slug: string) => {
    ...
    <>
      ...
      <section className="pt-32 pb-16 px-4 sm:px-6 lg:px-8 bg-gradient-to-br from-therapy-50 to-cream">
        <div className="max-w-7xl mx-auto text-center">
          <h1 className={headingVariants({ size: 'h1', align: 'center' })}>
            Nasi Terapeuci
          </h1>
      ...
      <section className="py-8 bg-white border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-wrap gap-4 justify-center">
            <button className={buttonVariants({ variant: 'primary', size: 'base' })}>
              Wszyscy
            </button>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {therapists.map((therapist: Therapist) => ( // Dodaj typ Therapist
                <Link
                  ...
                  onMouseEnter={() => prefetchTherapist(therapist.slug)}
                  onFocus={() => prefetchTherapist(therapist.slug)}
                >
                      ...
                      {therapist.specializations && therapist.specializations.length > 0 ? (
                        <>
                          {therapist.specializations.slice(0, 3).map((spec: string, index: number) => (
                            <span 
                              key={index}
                              className="text-xs bg-sage-light/20 text-sage-dark px-3 py-1 rounded-full font-medium"
                            >
                              {spec}
                            </span>
                          ))}
